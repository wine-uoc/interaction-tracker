...
if ACC_CONST:
    ...
    ax = np.append([], [0.0] * (max_time // 4))

    ax = np.append(ax, np.arange(0, MAX_ACC, MAX_ACC / (max_time / 8.0)))
    ax = np.append(ax, np.arange(0, -MAX_ACC, -MAX_ACC / (max_time / 8.0)))

    ax = np.append(ax, [0.0] * (max_time // 4))
    ...
    ay = np.append([], np.arange(0, MAX_ACC, MAX_ACC / (max_time / 8.0)))
    ay = np.append(ay, np.arange(0, -MAX_ACC, -MAX_ACC / (max_time / 8.0)))

    ay = np.append(ay, [0.0] * (max_time // 4))
...
for t in TIME:
    if ACC_CONST:
        x.append(x[t - 1] + (vx[t - 1] * delta_t) + ((ax * (delta_t ** 2)) / 2))
        y.append(y[t - 1] + (vy[t - 1] * delta_t) + ((ay * (delta_t ** 2)) / 2))
        vx.append(vx[t - 1] + (delta_t * ax))
        vy.append(vy[t - 1] + (delta_t * ay))
    else:
...
for i in list(zip(x,y,vx,vy,ax,ay)):
    print(i)
...
for n in enumerate(noise_acc):
    if ACC_CONST:
        accx_dirt_pred.append(ax + n[1][0])
        accy_dirt_pred.append(ay + n[1][1])
    else:
